...
@mcp.tool()
async def get_service_spend_last_30_days() -> Union[str, None]:
    """
    Retrieve daily spend by service over the last 30 days.
    
    Returns:
        Union[str, None]: A string containing the formatted output of daily service costs, 
        or None if an error occurs or no data is found.
    """
    # Initialize the Cost Explorer client
    ce_client = boto3.client('ce')
    
    # Calculate the time period - last 30 days
    end_date = datetime.now().strftime('%Y-%m-%d')
    start_date = (datetime.now() - timedelta(days=30)).strftime('%Y-%m-%d')
    
    # Create a string buffer to store all outputs
    output_buffer = []
    ...
    try:
        ...
        output_buffer.append(f"\nDaily Service Spend from {start_date} to {end_date}:")
        ...
        for date in all_dates:
            ...
            for service in all_services:
                row[service] = daily_costs[date].get(service, 0.0)
            df_data.append(row)
        ...
        if df_data:
            ...
            service_totals = df.drop('Date', axis=1).sum().sort_values(ascending=False)
            
            # Display top services by total cost
            top_services = service_totals.head(10).index.tolist()
            display_columns = ['Date'] + [s for s in top_services if s != 'Total'] + ['Total']
            
            output_buffer.append("Top 10 Services by Cost (Last 30 Days):")
            output_buffer.append(tabulate(df[display_columns].round(2), headers='keys', tablefmt='pretty', showindex=False))
            
            output_buffer.append("\nTotal Cost by Service (Last 30 Days):")
            output_buffer.append(tabulate(pd.DataFrame({
                'Service': service_totals.index,
                'Cost': service_totals.values.round(2)
            }).head(10), headers='keys', tablefmt='pretty', showindex=False))
            
            # Note on currency
            output_buffer.append(f"\nNote: All costs are in {currency if 'currency' in locals() else 'USD'}")
            
            # Join all buffer items into a single string
            return "\n".join(output_buffer)
        else:
            output_buffer.append("No cost data found for the specified period")
            return "\n".join(output_buffer)
    
    except Exception as e:
        output_buffer.append(f"Error retrieving service cost data: {str(e)}")
        return "\n".join(output_buffer)
